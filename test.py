...
def find_jobs_from(website, job_title, jobtype, desired_characs, filename="results.xls"):
    """
    ...
    if website == 'Glassdoor':
        location_of_driver = 'E:\API Testing\Selenium\Drivers'
        driver = initiate_driver(location_of_driver, browser='chrome')
        job_soup = load_glassdoor_jobs_div(job_title, jobtype,driver)
    ...
    if website == 'CWjobs':
        location_of_driver = 'E:\API Testing\Selenium\Drivers'
        driver = initiate_driver(location_of_driver, browser='chrome')
        job_soup = make_job_search(job_title, jobtype, driver)
...
def load_glassdoor_jobs_div(job_title,jobtype,driver):
    getVars = {'sc.keyword': job_title, 'jobType': jobtype, 'fromAge': '1'}
    # https: // www.glassdoor.com / Job / postman - api - jobs - SRCH_KO0, 11.
    # htm?jobType = fulltime & fromAge = 1
    url = ('https://www.glassdoor.com/Job/index.htm?' + urllib.parse.urlencode(getVars))
    driver.get(url)
...
def initiate_driver(location_of_driver, browser):
    if browser == 'chrome':
        driver = webdriver.Chrome(executable_path="E:\API Testing\Selenium\Drivers\chromedriver.exe")
